...
function initializeGameData(data) {
    ...
    return data.map(game => {
        ...
        const stake = determineStake(evToToBet);
...
function trackVisitors() {
    ...
    // 1. Increment Total Views (Happens on every page load/refresh)
    globalTotalViews++;
...
function initializeDashboard() {
    // 1. Initial Data Processing (Uses current mock_data without API cost)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 1. INITIAL SETUP
    trackVisitors(); // Only tracks visitors/views
    ...
    // CRITICAL FIX: Initializes the dashboard with current data (0 API cost on load)
    initializeDashboard();
